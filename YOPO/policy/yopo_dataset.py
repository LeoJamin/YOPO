...
class YOPODataset(Dataset):
    def __init__(self, mode='train', val_ratio=0.1):
        ...
        # --- 修改位置 1：增加 payload_states 和 payload_params 容器 ---
        self.img_list, self.map_idx, self.positions, self.quaternions = [], [], np.empty((0, 3), dtype=np.float32), np.empty((0, 4), dtype=np.float32)
        self.payload_states = np.empty((0, 4), dtype=np.float32) # theta, phi, d_theta, d_phi
        self.payload_params = np.empty((0, 2), dtype=np.float32) # length, mass
        ...
        # Load physically consistent payload state library (generated by pendulum_simulator.py)
        payload_lib_path = os.path.join(data_dir, "payload_state_library.npy")
        if os.path.exists(payload_lib_path):
            self._payload_lib = np.load(payload_lib_path)  # (N, 6): [theta, phi, dtheta, dphi, L, m]
            print(f"Loaded payload state library: {self._payload_lib.shape[0]} states from ODE simulation")
        else:
            self._payload_lib = None
            print("WARNING: payload_state_library.npy not found. "
                  "Run 'python policy/pendulum_simulator.py --mode generate' first. "
                  "Falling back to random payload states (physically inconsistent).")
        ...
        for data_idx in range(len(datafolders)):
            ...
            df = pd.read_csv(data_dir + f"/pose-{data_idx}.csv")
            # Pad to 13 columns if dataset was generated without payload columns
            while df.shape[1] < 13:
                df[f'_col{df.shape[1]}'] = 0.3 if df.shape[1] == 12 else 0.8
            states = df.iloc[:, :13].values.astype(np.float32)
            # Align image list and CSV rows (data collection may have been interrupted)
            n = min(len(image_file_names), len(states))
            image_file_names = image_file_names[:n]
            states = states[:n]
            # --- 修改位置 2：切分出新增的负载状态和参数列 ---
            positions = states[:, 0:3]
            quaternions = states[:, 3:7]
            payload_states = states[:, 7:11]  # theta, phi, d_theta, d_phi
            payload_params = states[:, 11:13] # length, mass

            # --- 修改位置 3：更新 train_test_split 以包含新增数据 ---
            file_names_train, file_names_val, positions_train, positions_val, quaternions_train, quaternions_val, payload_train, payload_val, params_train, params_val = train_test_split(
                image_file_names, positions, quaternions, payload_states, payload_params, test_size=val_ratio, random_state=0)
            ...
            if mode == 'train':
                ...
                # --- 修改位置 4a：存储训练集负载数据 ---
                self.payload_states = np.vstack((self.payload_states, payload_train.astype(np.float32)))
                self.payload_params = np.vstack((self.payload_params, params_train.astype(np.float32)))
                ...
                # --- 修改位置 4b：存储验证集负载数据 ---
                self.payload_states = np.vstack((self.payload_states, payload_val.astype(np.float32)))
                self.payload_params = np.vstack((self.payload_params, params_val.astype(np.float32)))
        ...
        # --- 新增打印信息 ---
        print(f"{'Payload'     :<12} | Count: {self.payload_states.shape[0]:<3} |  State: {self.payload_states.shape[1]}, Param: {self.payload_params.shape[1]}")
        print("==================================================")
    ...
    def __getitem__(self, item):
        ...
        # 4. Sample payload state from physically consistent ODE library (training only)
        #    Validation uses deterministic CSV values for reproducible evaluation.
        #    L and m are fixed (unknown at deployment — network should not depend on them).
        if self._payload_lib is not None and self._mode == 'train':
            lib_idx = np.random.randint(0, len(self._payload_lib))
            lib_entry = self._payload_lib[lib_idx]  # [theta, phi, dtheta, dphi, L, m]
            p_state = lib_entry[:4].astype(np.float32)
        else:
            p_state = self.payload_states[item]
        # Fixed physical parameters — loss function uses these for physics
        # but network never sees them (13D input excludes L and m)
        p_params = np.array([cfg["payload"]["cable_length"], cfg["payload"]["mass"]], dtype=np.float32)

        # vel & acc & goal are in body frame, NWU, and no-normalization
        return image, self.positions[item], rot_wb, random_obs, p_state, p_params, self.map_idx[item]
    ...
    def _get_random_state(self):
        for _ in range(1000):
            ...
            raise RuntimeError("Failed to sample valid velocity after 1000 attempts")
        ...
        for _ in range(1000):
            ...
            raise RuntimeError("Failed to sample valid acceleration after 1000 attempts")
        return vel, acc
