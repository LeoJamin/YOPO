"""
Single-method ROS evaluation. Runs one method on the shared goal set.
Usage: python eval_ros_single.py --trial 45 --obs_dim 13 --epoch 20 --name "YOPO-45"
"""
...
def run_ep(trial, obs_dim, epoch, goal, ep_id):
    log_file = f"/tmp/yopo_single_t{trial}_ep{ep_id}.log"
...
def main():
    ...
    name = args.name or f"YOPO_{args.trial}"
    print(f"{'='*60}")
    print(f"Evaluating: {name} (trial={args.trial}, epoch={args.epoch}, obs_dim={args.obs_dim})")
    ...
    for gi, goal in enumerate(GOALS):
        ...
        print(f"  Goal {gi+1}/10: ({goal[0]:.0f},{goal[1]:.0f}) {gd:.0f}m {ga:.0f}°")
        kill_all(); start_ros()
        r = run_ep(args.trial, args.obs_dim, args.epoch, goal, gi+1)
    ...
    # Summary
    arr = sum(1 for r in results if r["arrived"])
    pk = [r["peak_swing"] for r in results]
    mn_sw = [r["mean_swing"] for r in results]
    cr = [r["cruise_swing"] for r in results if r["cruise_swing"]>0]
    print(f"\n{'='*60}")
    print(f"RESULTS: {name}")
    print(f"{'='*60}")
    print(f"  Success: {arr}/{len(results)} ({100*arr/len(results):.0f}%)")
    print(f"  Peak Swing:   {np.mean(pk):.1f}° ± {np.std(pk):.1f}°")
    print(f"  Avg Swing:    {np.mean(mn_sw):.1f}° ± {np.std(mn_sw):.1f}°")
    print(f"  Cruise Swing: {np.mean(cr):.1f}° ± {np.std(cr):.1f}°" if cr else "  Cruise Swing: N/A")
    print(f"  Mean Min Alt: {np.mean([r['min_z'] for r in results]):.2f}m")

    out = f"{YOPO_DIR}/saved/YOPO_{args.trial}/eval_results.json"
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out,'w') as f: json.dump({"name":name,"results":results},f,indent=2,default=str)
